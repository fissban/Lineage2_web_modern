...
$result = $db->executeQuery("SELECT count(id) AS size FROM auction_items_sold WHERE $condition");
while ($row = $result->fetch())
{
...
$result = $db->executeQuery("SELECT * FROM auction_items_sold WHERE $condition LIMIT $pageMin , $pageMax");
while ($row = $result->fetch())
{
